...
void JitLLVM::reg_imm(LLVMFunction* func, u32 d, u32 a, bool binary, u32 value,
                      Instruction::BinaryOps opc, bool Rc)
{
  IRBuilder<>* builder = func->GetBuilder();
...
void JitLLVM::addmex(LLVMFunction* func, UGeckoInstruction inst)
{
  ...
  func->StoreCarry(
      HasCarry(func, reg_ra, builder->CreateSub(old_carry_value, builder->getInt32(1))));
...
void JitLLVM::subfmex(LLVMFunction* func, UGeckoInstruction inst)
{
  ...
  // SetCarry(Helper_Carry(~a, carry - 1));
  Value* has_carry =
      HasCarry(func, not_ra, builder->CreateSub(old_carry_value, builder->getInt32(1)));
  func->StoreCarry(has_carry);
