    ...
    : m_builder(mod->getContext())
{
  ...
  m_func = Function::Create(type, Function::ExternalLinkage, func_name, m_mod);
    ...
    : m_builder(mod->getContext())
{
  ...
  // Generate the function
  m_func = Function::Create(type, Function::ExternalLinkage, name, m_mod);
  m_func->setCallingConv(CallingConv::C);
...
LoadInst* LLVMFunction::GetGlobalMemory()
{
  if (!m_ptrs.m_memory_base)
  {
    ...
    m_ptrs.m_memory_base =
        m_builder.CreateLoad(memory_global, m_builder.getInt32(0), "memory_base");
  }
...
LoadInst* LLVMFunction::GetGatherPipe()
{
  if (!m_ptrs.m_gatherpipe)
  {
    ...
    m_ptrs.m_gatherpipe =
        m_builder.CreateLoad(gatherpipe_global, m_builder.getInt32(0), "gatherpipe");
  }
...
LoadInst* LLVMFunction::GetGatherPipeCount()
{
  if (!m_ptrs.m_gatherpipe_count)
  {
    ...
    m_ptrs.m_gatherpipe_count =
        m_builder.CreateLoad(gatherpipe_count_global, m_builder.getInt32(0), "gatherpipe_count");
  }
...
LoadInst* LLVMFunction::LoadSR(Value* sr)
{
  ...
  const std::vector<Value*> loc = {
      m_builder.getInt32(0),  // 0th index in to the PowerPCState "array"
      m_builder.getInt32(INDEX_SR), sr};
