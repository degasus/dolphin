...
void JitLLVM::FallBackToInterpreter(LLVMFunction* func, UGeckoInstruction inst)
{
  ...
  if (!bound_func)
  {
    ...
    FunctionType* type = GenerateFunctionType(Type::getVoidTy(con), Type::getInt32Ty(con));

    bound_func = Function::Create(type, Function::ExternalLinkage, opcodeInfo->opname,
                                  m_cur_mod->GetModule());
