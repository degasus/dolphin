                                       ...
                                       u8* out_ptr, bool exception_list)
{
  ...
  case CompressionType::Purge:
  {
    ...
    while (offset_in_data < hash_offset)
    {
      ...
      purge_segment.offset = Common::swap32(purge_segment.offset);
      purge_segment.size = Common::swap32(purge_segment.size);

      if (purge_segment.offset < offset_in_decompressed_data)
        return false;

      const u32 blank_bytes = purge_segment.offset - offset_in_decompressed_data;
      std::memset(out_ptr, 0, blank_bytes);
      out_ptr += blank_bytes;

      if (purge_segment.size != 0 && !m_file.ReadBytes(out_ptr, purge_segment.size))
        return false;
      out_ptr += purge_segment.size;

      offset_in_data += sizeof(PurgeSegment) + purge_segment.size;
      offset_in_decompressed_data = purge_segment.offset + purge_segment.size;
    }
...
std::optional<u64> WIAFileReader::ReadExceptionListFromFile()
{
  ...
  const u64 exception_size = Common::AlignUp(
      sizeof(exceptions) + Common::swap16(exceptions) * sizeof(HashExceptionEntry), 4);

  if (!m_file.Seek(exception_size - sizeof(exceptions), SEEK_CUR))
    return std::nullopt;
